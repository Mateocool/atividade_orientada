...
fig <- plot_ly(df.multi_solo, x = ~`percentual corrigido`, y = ~umidade, z = ~temperatura, type = 'scatter3d', size=1, color = ~Prevenção)
...
A altura média das couves pode ser conferida na figura abaixo, Figura \@ref(fig:fig8):
...
```{r fig10, echo=FALSE, fig.cap="Teste Tukey para o tamanho das couves. Fonte o autor, Porto Alegre - RS, 2021"}
...
knitr::kable(
  head(df.media_pulgoes_dia[, 1:2], 5), longtable = TRUE, booktabs = TRUE,
...
knitr::kable(
  head(tabela5[, 1:5], 4), longtable = TRUE, booktabs = TRUE,
