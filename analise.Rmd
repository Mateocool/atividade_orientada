---
title: Reavaliação de _biomulching_ com agregação de extrato de _Capsicum frutescens_ como cobertura de solo e agente de controle de pragas em _Brassica oleracea_
author: 
- Mateus Colombo^[Graduando em Biotecnologia - Instituto de Biociências, Universidade Federal do Rio Grande do Sul, mateus.colombo@ufrgs.br]
- Lucas Mello Schnorr^[Professor orientador - Instituto de Informática, Universidade Federal do Rio Grande do Sul, schnorr@inf.ufrgs.br]
date: "Novembro/2021"
output:
...
Um aspecto bastante importante sobre os agrotóxicos é de que eles afetam a vida como um todo. De uma maneira positiva pois auxilia no combate de pragas e protege as plantações contra perdas econômicas, sendo como uma forma de segurança para os produtores. Contudo, os agrotóxicos também irão afetar todos os outros seres vivos presentes no meio ambiente, os quais podem ser benéficos para as plantas, os seres humanos e a natureza.

Os agrotóxicos quando mal administrados pelos produtores podem causar uma série de malefícios à saúde humana, como intoxicação, desordens hormonais, doenças neurológicas e até mesmo alguns tipos de câncer podem ser favorecidos pela exposição desprotegida e excessiva aos agrotóxicos, [@cassal2014agrotoxicos]. Além de tamanhos prejuízos à saúde, também há vários problemas ambientais que podem ser causados pelo uso indiscriminado destas substâncias. Entre eles estão a poluição do solo, da água e do ar, recursos que consequentemente afetam os ecossistemas e a biodiversidade expostas a tais contaminantes provocando a diminuição da biodiversidade e o aparecimento de pragas resistentes, [@soares2007agricultural], [@cassal2014agrotoxicos]. 
...
Naquele trabalho concluiu-se que o _biomulching_ poderia ser empregado como cobertura de solo e que a essência de pimenta malagueta apresenta potencial para repelir pulgões, entretanto, não foi possível verificar a eficiência do método contra as pragas da couves devido a problemas no modelo experimental. Outro ponto a resaltar foi o de que a cor da cobertura utilizada poderia influenciar na severidade da infestação de pragas, pois como foi observado, no canteiro com cobertura de lona amerela houve um menor número de insetos capturados nas armadilhas de amostragem. Tais conclusões foram apresentadas em diversas feiras nacionais e internacionais de iniciação científica como MOSTRASEG, MOSTRATEC, FEBRACE, MCTEA e GENIUS Olympiad.

Na parte de análise do projeto do _biomulching_, uma grande quantidade de dados sobre diversas variáveis e testes foram obtidas e processadas muitas vezes manualmente por meio de anotações em cadernos. Após os dados eram passados para tabelas preenchidas manualmente no software Microsoft Excel, estas foram divididas em diferentes pastas e grupos de dados de uma forma que facilitasse a sua visualização. Quando se encerou a parte experimental, realizou-se a análise dos dados adquiridos performando cálculos, testes estatísticos e criando gráficos dentro do próprio software. Para tanto os dados eram selecionados diretamente nas tabelas ou então eram geradas novas tabelas apenas com os dados de interesse, o que ocasionava uma grande manipulação dos dados. E, depois de todos esse processo ainda era necessário passar os resultados e gráficos para o relatório cujo estava sendo redigido em outro software.

A análise manual dos dados via Excel poderia gerar conclusões imprecisas e indefinidas sobre o objeto de estudo, um problema conhecido na comunidade científica. Alguns exemplos que já ocorreram no passado são:

- A autocorreção que o Excel realiza nas células, como alteração de números para datas, pode corromper os dados brutos alterando-os. No ramo da genética esse problema foi diagnosticado em 2004, quando perceberam que cerca de 30 nomes de genes e proteínas humanas eram alteradas a datas, [@ziemann2021], [@zeeberg2004mistaken].

- Na área econômica, um artigo agora infame dos economistas de Harvard Carmen Reinhart e Kenneth Rogoff foi usado para justificar os cortes de austeridade após a crise financeira global, mas a análise continha um erro crítico do Excel que levou à omissão de cinco dos 20 países em sua modelagem, [@ziemann2021], [@reinhart2010growth]. Alguns anos depois da publicação do artigo um estudante de doutorado revisou o trabalho e encontrou os problemas dos dados, [@herndon2014does].

- Na pesquisa biomédica, um erro na preparação de uma pasta de amostra resultou em um conjunto completo de rótulos de amostra sendo deslocados em uma posição e mudando completamente os resultados da análise genômica. Esses resultados foram significativos porque estavam sendo usados para justificar os medicamentos que os pacientes deveriam receber em um ensaio clínico subsequente. Este pode ser um caso isolado, mas não sabemos realmente o quão comuns esses erros são em pesquisas devido à falta de estudos sistemáticos de detecção de erros, [@ziemann2021], [@hutson2010data].
...
Para a parte de organização e descrição dos processos do projeto, uma técnica que pode ser empregada é a de _literal programing_, ou programação literal, a qual foi criada por Donald Knuth e é semelhante a um caderno de anotações digital. Com a programação literal as informações sobre os conceitos, as ideias, os precedimentos, os resultados e como todos estes foram processados no programa, ficam descritos em um único lugar. Assim tanto a parte da pesquisa propriamente dita, quanto da análise, são descritas de uma forma explicativa, intercalando o sprit com trechos de códigos, que executam funções com os dados, com textos da linguagem humana, que explicam o foi feito e como foi feito. Assim tornando o script mais fácil de entender, tanto às pessoas que possuem maior conhecimento sobre pragramação quanto às que não dominam essa área [@knuth1984literate].

A seguir no relatório são descritos os objetivos do trabalho, a metodologia aplicada, os resultados obtidos através da análise pelo software Rstudio e, por fim, as conclusões a respeito do trabalho e da análise.

# Objetivos

## Objetivo geral

O objetivo do projeto é utilizar ferramentas computacionais como o Rstudio para reproduzir as respontas aos questionamentos a respeito do biomulching de forma reprodutível, ou seja, independente do equipamento utilizado e do analista que performa a análise.
...
```{r dado, echo=FALSE, fig.align = 'center', out.width= "80%", fig.cap="Fonte o autor, Porto Alegre - RS, 2021"}
...
```{r fig4, echo=FALSE, fig.align = 'center', out.width= "80%", fig.cap="Fonte o autor, Porto Alegre - RS, 2021"}
df.umidade_solo %>%
...
Para confirmar se há diferenças na umidade do solo entre os canteiro, foi realizada a análise de variancia (One-way ANOVA) e o teste de Tukey. Conforme a Figura \@ref(fig:fig5), não há diferenças significativas a 95% de confiança entre os valores da umidade do solo dos canteiros.

```{r fig5, echo=FALSE, fig.align = 'center', out.width= "80%", fig.cap="Teste Tukey para umidade do solo. Fonte o autor, Porto Alegre - RS, 2021"}
