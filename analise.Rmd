...
Visando instigar a elaboração de novos métodos de controle de pragas na agricultura por meio do método científico, o presente projeto irá revisar os dados obtidos em um trabalho desenvolvido pelo aluno Mateus Colombo durante o Ensino Médio com o uso de pacotes de linguagem R. Em seu trabalho foi criado o _biomulching_ com agregação de extratos de pimenta malagueta, uma técnica alternativa aos agrotóxicos que consiste como cobertura de solo e de prevenção e controle de pragas em couve. Esta técnica une o princípio da cobertura de solo com o dos bioinseticidas, criando um mulching biodegradável, a base de casca de laranja, com a agregação de extratos de _Capsicum frutensis_ para a liberação controlada de sustâncias inseticidas durante o cultivo. Assim, hipoteticamente, auxiliando as plantas para um melhor desenvolvimento e garantindo uma boa colheita com poucos prejuízos econômicos.

![Protótipo de Biomulching em plantação de couve](P_20181222_200022.jpg)

Um ponto importante no método científico, é a correta análise e descrição dos processos e dados obtidos. Na parta de análise, no projeto original, uma grande quantidade de dados sobre diversas variáveis e testes foram obtidas e processadas muitas vezes manualmente, o que poderia acabar gerando desorganização e erros, [@wickham2016r]. Isto ao final poderia, consequentemente, gerar conclusões imprecisas e indefinidas sobre o objeto de estudo. Através da análise correta dos dados obtidos dos experimentos, o trabalho de pesquisa ganha maior reprodutibilidade e confiança ao estudo que foi realizado. Uma proposta de amenizar os problemas causados na análise de dados por erros humanos é a utilização de softwares e outros programas de análise tais como o Rstudio.
...
O objetivo do projeto é utilizar ferramentas computacinais como o Rstudio para verificar se conseguimos responder os questionamentos a respeito do biomulching de forma reprodutível. Avaliando se o biomulching pode ser utilizado como cobertura de solo e se auxilia no combate pragas na cultura da couve.

método científico e tal, um negócio assim, e mais não sei o que...

da para colocar uma coisa mais específica e direta. um parágrafo é o suficiente.
...
Segue o fluxograma com todas as etapas de desenvolvimento do projeto de pesquisa:
...
```{r dado, echo=FALSE, fig.cap="Fonte o autor, Porto Alegre - RS, 2021"}
...
Para confirmar se há diferenças na umidade do solo entre os canteiro, foi realizada análise de variancia (One-way ANOVA) e o teste de Tukey. Conforme a Figura \@ref(fig:fig5), onde consta o resultado do teste de Tukey, não há diferenças significativas a 95% de confiança entre os valores da umidade do solo dos canteiros.
...
A altura média das couves pode se conferida na figura abaixo, Figura \@ref(fig:fig8):
...
```{r pulgoes, echo=FALSE, fig.cap="Fonte o autor, Porto Alegre - RS, 2021"}
...
A quantidade de outros insetos capturados nas armadilhas pode ser observada na Figura \@ref(fig:outros). Os pontos apresentados possuem certa similaridade com os da Figura \@ref(fig:pulgoes), pois possuem picos e regiões de queda parecidas e também se verifica que com o passar do tempo a quantidade de insetos capturados aumenta. A lona amarela, teve um resultado parecido com o do número de pulões e também mostrou um menor número de outros insetos.
