...
#aov(lm(`percentual corrigido`~Prevenção + temperatura + umidade, data= df.multi_solo)) -> solo_multianova

#summary(solo_multianova)

df.multi_solo %>%
  ggplot(seals, aes(temperatura = long, umidade = lat)) +
  geom_contour(aes(z = `percentual corrigido`))
...
df.plantas_invasoras %>%
  rename(dia = Dia) %>%
  left_join(df.clima, by = "dia") -> df.multi_invasoras


aov(lm(Quantidade~Prevenção + temperatura + umidade, data= df.multi_invasoras)) ->invasoras_anova

summary(invasoras_anova)
...
**Em dias quentes e de pouca umidade como estava o ritmo de degradação do _biomulching_ durante esses períodos?**
