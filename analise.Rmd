...
knitr::kable(
  head(tabela1[, 1:4], 3),  longtable = TRUE, booktabs = TRUE,
...
```{r fig2, echo=FALSE,  out.width= "80%", fig.cap="Fonte: Estação Meteorológica do aeroclube, Garibaldi - RS, 2018/2019"}
...
```{r fig3, echo=FALSE, out.width= "80%", fig.cap="Fonte: Estação Meteorológica do aeroclube, Garibaldi - RS, 2018/2019"}
...
```{r fig5, echo=FALSE, out.width= "70%", fig.cap="Teste Tukey para umidade do solo. Fonte o autor, Porto Alegre - RS, 2021"}
...
fig <- plot_ly(data, x = ~x, y = ~y, z = ~z, type = 'scatter3d', size=0.5)
...
**trÊs estrelas códigos de significancia, explica a quantidade.

as estrelas são uma escala de influencias**


summary lm, modelo

valor abaixo de 0,9 é uma influencia fraca. 
...
df.plantas_invasoras %>%
  ...
  knitr::kable(
  head(tabela2[, 1:2], 3), longtable = TRUE, booktabs = TRUE,
  caption = 'Eficiência dos tratamentos'
)
...
knitr::kable(
  head(tabela3[, 1:4], 5), longtable = TRUE, booktabs = TRUE,
