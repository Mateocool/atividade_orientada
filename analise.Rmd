...
Dentre os malefícios destes, ao meio ambiente pode-se citar a contaminação do solo, água, ar, diminuição da biodiversidade, pragas resistentes, entre outros, além de tamanhos prejuízos à saúde, como intoxicação, câncer e diversas outras doenças causadas pela longa exposição a estas substâncias [@KIM2017525].
...
Estas substâncias quando má administradas pelos produtores pode causar uma série de malefícios à saúde humana, como intoxicação, desordens hormonais, doenças neurológicas e até mesmo alguns tipos de câncer podem ser favorecidos pela exposição desprotegida e excessiva aos agrotóxicos, [@cassal2014agrotoxicos]. Além de tamanhos prejuízos à saúde, também há vários problemas ambientais que podem ser causados pelo uso indiscriminado destas substâncias. Entre elas estão a poluição do solo, da água, do ar, e consequentemente isso irá afetar os ecossistemas e a biodiversidade expostas a tais contaminantes [@soares2007agricultural], [@cassal2014agrotoxicos]. 
...
Para o monitoramento da população de pulgões alados e outros insetos foi utilizado o método de armadilha com bandejas de água desenvolvida por Moerick, [@resende2007amostragem]. As bandejas foram pintadas de amarelo, para atrair os insetos foi usado o estímulo visual da radiação amarela refletiva.

![Armadilha utilizada no controle do número de pulgões alados](P_20190105_174536.jpg)
...
Também se verificou que nas amostras de controle e do tratamento com o _biomulching_ ocorreu uma proliferação de pulgões após 48horas, aumentando o número total de pulgões vivos nas amostras conforme evidencia a figura abaixo  @ref(fig:fig1):

```{r fig1, echo=FALSE, fig.cap="Fonte o autor, Porto Alegre - RS, 2021"}
...
```{r tab1, echo=FALSE, fig.cap="Tabela 1. Fonte o autor, Porto Alegre - RS, 2021"}
df.controlado %>%
  filter(`Tempo (h)` == "116") %>%
...
Os dados relativos à temperatura média e a umidade relativa do ar registrada diariamente às 14:00hrs durante o período de realização do estudo (17 de dezembro de 2018 a 11 de março de 2019), são apresentados nas /@ref(fig:fig2) e /@ref(fig:fig3). A temperatura média do ar durante o período experimental foi de 25,22°C.

```{r fig2, echo=FALSE, fig.cap="Fonte: Estação Meteorológica do aeroclube, Garibaldi - RS, 2018/2019"}
df.clima%>%
...
```{r fig3, echo=FALSE, fig.cap="Fonte: Estação Meteorológica do aeroclube, Garibaldi - RS, 2018/2019"}
...
```{r fig4, echo=FALSE, fig.cap="Fonte o autor, Porto Alegre - RS, 2021"}
df.umidade_solo %>%
...
```
Para confirmar se há diferenças na umidade do solo entre os canteiro, foi realizada análise de variancia (One-way ANOVA) e o teste de Tukey. Conforme a  /@ref(fig:fig5), onde consta o resultado do teste de Tukey, não há diferenças significativas a 95% de confiança entre os valores da umidade do solo dos canteiros.

```{r fig5, echo=FALSE, fig.cap="Teste Tukey para umidade do solo. Fonte o autor, Porto Alegre - RS, 2021"}
...
```
```{r tab2, echo=FALSE, fig.cap="Tabela 2. Fonte o autor, Porto Alegre - RS, 2021"}
...
```{r degradabilidade, echo=FALSE, fig.cap="Fonte o autor, Porto Alegre - RS, 2021"}
...
As plantas se desenvolveram com saúde e bem viçosas. Apenas uma planta morreu, a X2 no canteiro com lona preta, e tiveram 2 casos em que as couves não se desenvolveram. Uma no canteiro de contraprova, a T5, a qual foi impedida de crescer devido a muitas plantas invasoras próximas a ela, o que diminuiu a o tempo de exposição ao sol. E a outra no canteiro com lona amarela, a S3, a qual teve dificuldade de se adequar à cobertura de plástico, devido a fatores como umidade em excesso ou em falta.

#### Altura média das couves

A altura média das couves pode se conferida na figura abaixo \@ref[fig:fig8]:

Pode-se ver que não houveram grandes diferenças em relação à altura das couves. Sendo o canteiro de pimenta o com a maior média de altura, seguido do _biomulching_, lona amarela, contraprova e lona preta. A baixa estatura das plantas em X pode ser devido ao fato de elas terem sido plantadas duas semanas após as demais. Outro fator que contribuiu para alguns canteiros terem se desenvolvido mais, foi o tempo de insolação, um parâmetro não aferido neste experimento. Os canteiros de lona amarela, pimenta, e contraprova recebiam mais horas de sol que em comparação com 0s de _biomulching_ e lona preta.

```{r fig8, echo=FALSE, fig.cap="Fonte o autor, Porto Alegre - RS, 2021"}
...
Também foi verificado que, como as plantas do canteiro de _biomulching_ conseguiram se desenvolver de forma saudável, o uso do _biomulching_ não é prejudicial às plantas, ou seja, não é tóxico.
...
```{r fig9, echo=FALSE, fig.cap="Fonte o autor, Porto Alegre - RS, 2021"}
df.dados_plantas%>%
...
O gráfico indica que não há diferença estatística no tamanho das cauves entre os diferentes canteiros, o que indica que nenhum dos tratamentos as plantas tiveram dificuldade para se desenvolver, fora aqueles casos específicos anteriormente citados. Mesmo assim, fizemos o teste estatístico, teste de Tukey, para confirmar par a par se existe ou não tal diferença:

```{r fig10, echo=FALSE, fig.cap="Fonte o autor, Porto Alegre - RS, 2021"}
...
Selecionamos um indivíduo aleatoriamente da lona preta para ter a mesma quantidade de indivíduos:

```{r calculos, fig.cap="Fonte o autor, include=FALSE}
set.seed(1)
df.dados_plantas%>%
...
Pode-se notar que mesmo o canteiro de contraprova ter sido o mais infestado por pulgões, apresentar o maior número de insetos capturados nas armadilhas e ter apenas 4 plantas produzindo, ainda sim, foi o que mais produziu massa comestível, seguido por pimenta, lona amarela, _biomulching_ e lona preta. Este fato, pode estar relacionado ao de que os canteiros de contraporva, pimenta e lona amarela estavam mais protegidos, de um lado havia uma parede e de outro os canteiros de _biomulching_ e lona preta. Assim os canteiros de _biomulching e lona preta acabaram sendo prejudicados, uma por terem algumas horas a menos de insolação e outra por não terem tido a mesma proteção que os demais canteiros.
...
O rendimento de cada couve foi calculado a partir da relação de sua massa total (caule e folhas) pela massa consumível (folhas saudáveis e boas). Segue abaixo o rendimento dos pés de couve em cada canteiro \@ref[fig:tab3].

```{r tab3, echo=FALSE, fig.cap="Fonte o autor, Porto Alegre - RS, 2021"}

df.plantas_selecionadas_dados_totais %>%
  pivot_wider(names_from=Observação, values_from=Massa.Total) %>%
  mutate(Rendimento = `massa comestível (g)`/`massa total (g)` * 100) %>%
  select(Rendimento, everything())
```

O canteiro com o maior rendimento foi o de contraprova, o qual também havia produzido mais massa consumível, seguido por lona preta, pimenta, _biomulching_ e lona amarela. O canteiro com lona amarela foi o que menos rendeu devido ao fato de terem plantas muito infestadas por pulgões, de modo que não era possível utilizar as folhar para consumo.
A média geral de rendimento do experimento foi de 40,9%
...
Em relação aos pulgões, foram encontrados em sua maioria _Brevicoryne brassicae_, além de também terem sido encontradas outras duas espécies descritas na bibliografia as quais são comuns na cultura da colve, _Myzus persicae_ e _Lipaphis erisinii_. As três espécies foram identificadas em todos os canteiros.

A quantidade de pulgões alados capturados nas armadilhas ao longo do tempo pode ser conferida na figura abaixo \@ref[fig:pulgões]:

```{r pulgões, echo=FALSE, fig.cap="Fonte o autor, Porto Alegre - RS, 2021"}
...
```
De acordo com os dados obtidos, o canteiro de contraprova foi o qual teve maior incidência de pulgões alados capturados nas armadilhas, seguido por pimenta, _biomulching_, lona preta e lona amarela. O canteiro de pimenta, o qual foi aplicado a essência de pimenta, não teve uma quantidade menor de pulgões devido ao fato de ser ter feito a aplicação da essência da pimenta apenas durante uma semana no final do 1º mês, o que não foi o suficiente. O canteiro de lona amarela teve menos pulgões capturados nas armadilhas que todos os outros tratamento durante quase todo o período do teste. O que pode ser devido principalmente a coloração amarela do mulching utilizado neste canteiro, o que refletia mais luz por baixo das folhas. Entretanto conforme verificado anteriormente na questão sobre a massa produzida por este canteiro, uma parcela considerável não pôde ser consumida devida a alta infestação de colônias de pulgões nas folhas das plantas. Isso gera uma inconsistencia dos dados obtidos, o que precisa ser revisado pois a utilização da lona amarela pode ter afetado o princípio de funcionamento das armadilhas, as quais se baseia na reflexão da cor amarela para atrair insetos.

O canteiro com _biomulching_ não apresentou a menor quantidade de pulgões quando comparado a lona amarela e a lona preta. Como o canteiro de lona preta foi plantado duas semanas após os demais, este provavelmente poderia superar o número de pulgões do canteiro com _biomulching_, tanto que houveram diversas contagens em que os valores da lona preta foram superiores ao do _biomulching_, conforme evidencia-se na \@ref[fig:pulgões].

Para verificar a confiabilidade dos dados e se há ou não diferenças entre os tratamentos, foi calculado o valor médio de pulgões por dia capturados nos diferentes canteiros \@ref[fig:tab4].

```{r tab4, echo=FALSE, fig.cap="Fonte o autor, Porto Alegre - RS, 2021" }
dados_pulgoes %>%
  group_by(Prevenção) %>%
  ...
  summarize(Media = mean(Q)) %>%
  print -> df.media_pulgoes_dia
```

Após, os dados de pulgões por dia foram analisados estatisticamente por meio de análise de variância e teste de Tukey. O resultado do teste de Tukey pode ser conferido na \@ref[fig:pulgoes_anova]. Conforme o teste de Tukey, observa-se que há diferenças estatísticas entre o canteiro S (lona amarela) e os demais canteiros. Como relatado anteriormente, ponto que necessita de maiores estudos para compreender a razão deste fenomeno. Em relação aos demais canteiros, não há diferenças estatísticas entre os valores encontrados a 95% de confiança. Ou seja, não podemos dizer se há benefícios utilizando algum tratamento ou não quando comparados com a contraprova e entre si mesmos.

```{r pulgoes_anova, echo=FALSE, fig.cap="Teste de Tukey para pulgões. Fonte o autor, Porto Algre - RS, 2021"}
df.armadilhas %>%
...
A quantidade de outros insetos capturados nas armadilhas pode ser observada na \ref[fig:outros]. Os pontos apresentados possuem certa similaridade com os da \ref[fig:pulgoes], pois possuem picos e regiões de queda parecidas e também se verifica que com o passar do tempo a quantidade de insetos capturados aumenta. A lona amarela, teve um resultado parecido com o do número de pulões e também mostrou um menor número de outros insetos.

```{r outros, echo=FALSE, fig.cap="Fonte o autor, Porto Algre - RS, 2021"}
df.armadilhas %>%
...
```
Analisando a \ref[fig:outros_anova], pode-se visualizar uma tendencia para uma diferença maior no canteiro com lona amarela em relação aos demais canteiros, contudo não é possivil dizer que há diferenças significativas. Ou seja, não é possivél afirmar se o _biomulching_ nem o uso da essencia de pimenta contribui para a redução do número de outros insetos, bem como de pulgões.

```{r outros_anova, echo=FALSE, fig.cap="Fonte o autor, Porto Algre - RS, 2021"}
...
Sobre a presença de inimigos naturais, não foi registrado um grande número destes. Como pode-se ver na \@ref[fig:naturais], o canteiro que mais teve incidência de inimigos naturais nas armadilhas foi do _biomulching_. Por mais que este canteiro estivesse sobre tratamento, seu uso não influenciou de forma significativa na presença, nem na mortalidade de inimigos naturais.

```{r naturais, echo=FALSE, fig.cap="Fonte o autor, Porto Algre - RS, 2021"}
...
Embora os resultados não apresentem diferênças significativas, foi cálculado o percentual de eficiencia das diferentes prevenções contra pulgões, moscas brancas, lagartas e outros insetos em relação ao canteiro de controle utilizando a fórmula de Abbott, a qual pode ser conferida na seguinte equação:
...
```{r calculo_eficiencia, echo=FALSE, fig.cap="Eficiência contra diferentes pragas alvo. Fonte o autor, Porto Algre - RS, 2021"}
...
df.temp %>% filter(Prevenção != "Contraprova") %>%
  ...
  #mutate(`Praga alvo`= "Pulgões")-> df.eficiencia
...
df.temp %>% filter(Prevenção != "Contraprova") %>%
  ...
  arrange(-`Eficiência pulgões`)

```

Conforme vemos na tabela acima,o canteiro de lona amarela foi o que teve o menor número de pulgões e de outros insetos na contagem das armadilhas, atingindo uma eficiencia de 64% e 48% respectivamente. Entretanto nas aferições de lagartas e moscas que foram realizdas diretamente nas plantas, a lona amarela foi o canteiro com os piores resultados, sendo equivalente em relação à moscas e pior em relação à lagartas quando comparado ao canteiro de contraprova.

Os demais canteiros, como esperado, não apresentam grandes diferenças. Existe indícios de que a essência de pimenta pode ter reduzido a infestação de lagartas, mas não apresenta evidencias na redução de pulgões e outros insetos. O _biomulching_ reduziu a infestação de pulgões em 24%, porém não é eficiente contra a maioria dos outros insetos. 
...
A utilização de essências para a proteção das plantações e a utilização de bioplásticos como cobertura de solo já existe a algumas décadas, contudo não se encontrou nenhuma referência da existência de uma biocobertura com a agregação de essência com a finalidade de proteger as plantas, na internet, em livros, em revistas ou artigos científicos.

O objetivo do projeto foi atingido parcialmente. O objetivo de estudar as quatro plantas, alecrim, alho, sálvia e pimenta, para verificar possíveis efeitos inseticidas sobre o pulgão encontrado na couve foi alcançado. Sendo que após os testes terem sido realizados comprovou-se que a essência de pimenta foi a qual apresentou melhores resultados na plantação de couve e com uma eficiência de 100% em condições controladas. Assim, portanto, sendo eficiente no combate de pulgões.

Entretanto, em relação ao biomulching, este não apresentou os resultados desejáveis para o combate de pulgões e outras pragas das couves. Uma vez que nos primeiros testes, por causa da qualidade rudimentar dos protótipos compostos basicamente de materiais orgânicos não duraram o tempo ideal para acompanhar o crescimento da plantação. E nos últimos testes realizados em plantação de couve, o modelo de teste pode não ter contribuído para um resultado satisfatório do biomulching. Visto que os canteiros estavam bastante próximos uns dos outros, havia os que recebiam algum tratamento ao lado de outros que não estavam sendo tratados. Deste modo a presença de pulgões nas plantas próximas pode ter diminuído a eficiência do biomulching consideravelmente, bem como a ação da essência no canteiro P. Contudo ainda é possível verificar certa diminuição na presença de pulgões no canteiro com o uso do biomulching, assim indicando certa ação sobre os pulgões.

Sendo, o protótipo bioplástico 4, o qual obteve uma maior durabilidade, cerca de três meses, e apresentou os melhores resultados, com boas características físicas quando comparado aos outros protótipos compostos com casca de laranja, assim tornando-se o melhor protótipo já produzido, para o biomulching.

Por mais que não se tenha conseguido obter resultados os quais contribuíssem a comprovar as vantagens geradas pela agregação do extrato de pimenta no biomulching, os resultados atingidos são promissores. E este traz, por si só, benefícios e vantagens sobre o mulching convencional. Pois não gera resíduos e não é necessária sua retirada como o mulching convencional, evitando custos com o manejo e retirada dos resíduos. Apresentando resultados semelhantes ao mulching plástico, mantendo a umidade do solo, impedindo o crescimento de plantas invasoras e contribuindo para o crescimento da planta.

Além de contribuir para o ciclo de nutrientes, auxiliando na volta dos nutrientes ao solo, com a utilização de um resíduo agroindustrial, a casca de laranja. Assim agregando valor neste resíduo.

Se após mais testes ficar comprovado seu funcionamento e potencial para ser implantado em plantações com rendimento semelhante à dos agrotóxicos convencionais, este poderia ser uma nova opção para o combate de pragas. No qual o agricultor não terá de aplicar defensivos, pois já vão estar na plantação desde o início, na biocobertura, além de obter os mesmos benefícios da lona plástica.

Assim o projeto também contribui para alcançar as seguintes metas dos Objetivos de Desenvolvimento Sustentável (ODS) da ONU: Objetivo 2: Acabar com a fome, alcançar a segurança alimentar e melhoria da nutrição e promover a agricultura sustentável, objetivo 12: Assegurar padrões de produção e de consumo sustentáveis, e o objetivo 15: Proteger, recuperar e promover o uso sustentável dos ecossistemas terrestres, gerir de forma sustentável as florestas, combater a desertificação, deter e reverter a degradação da terra e deter a perda de biodiversidade.

A necessidade de criar um método de controle de pragas mais seguro, ecológico e sustentável que os agrotóxicos convencionais existe e é preciso! Pelo projeto nota-se que é possível a sua produção, trazendo assim maiores benefícios e facilidades, para quem produz, para o meio ambiente e para quem consome alimentos, estes que são a base da nossa existência.
