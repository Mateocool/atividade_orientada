...
Também se verificou a eficiência de cada método através da fórmula de Abbott, sendo os resultados apresentados na tabela abaixo, \@ref(tab:tab3):
...
knitr::kable(
  head(tabela1[, 1:4], 3), booktabs = TRUE,
...
df.plantas_invasoras %>%
  ...
  knitr::kable(
  head(tabela2[, 1:2], 3), booktabs = TRUE,
  caption = 'Eficiência dos tratamentos'
)
...
O rendimento de cada couve foi calculado a partir da relação de sua massa total (caule e folhas) pela massa consumível (folhas saudáveis e boas). Segue abaixo o rendimento dos pés de couve em cada canteiro \@ref[tab:tab3].
...
knitr::kable(
  head(tabela3[, 1:4], 5), booktabs = TRUE,
...
dados_pulgoes %>%
  ...
  summarize(Media = mean(Q)) -> df.media_pulgoes_dia

knitr::kable(
  head(df.media_pulgoes_dia[, 1:2], 5), booktabs = TRUE,
...
knitr::kable(
  head(tabela5[, 1:4], 4), booktabs = TRUE,
...
Agora vamos analisar as correlações entre os dados.
...
A localização das plantas interfere na presença de pulgões? não é possível responder pois isso não foi analisado. não temos os valores por planta, só por armadilha ou canteiro
...
Em relação ao canteiro que utilizou a essência de pimenta no controle e o _biomulching_, ouve alguma diferença? Eles tiveram desempenhos semelhantes? ouveram diferenças, o uso do biomulching é melhor do que apenas a aplicação da essência de pimenta
