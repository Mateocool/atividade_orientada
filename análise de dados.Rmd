---
title: Avaliação de biomulching com agregação de extrato de _Capsicum frutescens_ como
...
No presente projeto em principio foram reorganizados os dados do projeto original para facilitar a sua manipulação através do software. Os dados brutos foram obtidos através de testes e controle de armadilhas e variáveis em teste em campo do biomulching e também de um teste realizado em condições de laboratório para verificar como o principio ativo do biomulching, a essencia de pimenta, poderia se comportar em um teste com menos controle das variáveis como no caso do teste em condições de campo. 
...
![Fluxograma da metodologia](""C:/Users/Mateus Colombo/Documents/BIOTEC/3º semestre/Atividade orientada I/Slide1.JPG")
...
### Quantidade de plantas invasoras ao longo do período experimental
 ...
 Foi criada uma tibble para adicionar os dias em que foram realizadas as medições a cada semana. 
...
Foram importados os seguintes conjuntos dados os quais foram armazenados em "df.dados_plantas":
 ...
 Corresponde ao valor médio máxima das folhas de cada couve em cada um dos diferentes tratamentos ao final do período experimental. 
...
Inicialmente foi criado uma legenda para os dados com o intuito de melhorar a interpretação dos dados ao gerar os gráficos. Depois os dados foram reorganizados de forma que os dados de cada tratamento fossem reunidos em uma coluna.
...
A essência de pimenta por si só, auxilia no controle dos pulgões? E de moscas e lagartas?
...
df.armadilhas %>%
  # Vamos filtrar observações com as moscas
  filter(Agente == "mosca") %>%
  ggplot(aes(x = Prevenção, y = Quantidade))+
  geom_bar(stat = "identity")+
  ggtitle("Número total de moscas em cada canteiro")

df.armadilhas %>%
  # Vamos filtrar observações com as lagartas
  filter(Agente == "lagarta") %>%
  ggplot(aes(x = Prevenção, y = Quantidade))+
  geom_bar(stat = "identity")+
  ggtitle("Número total de lagartas em cada canteiro")

```
Conforme evidencia o gráfico 2, não há diferença visivel em relação ao uso da essencia de pimenta para o combate de pulgões. Contudo veríficamos no gráfico 1 uma diferença significativa no tratamento que utilizou o mulching convencional amarelo, o qual teve a menor contagem do número de pulgões. Em relação as moscas, esta esteve presente em todos os canteiros, sendo em menor quantidade nos canteiros com lona preta e pimenta. Também foi observado um menor número de lagarta nestes mesmos canteiros mais o canteiro com biomulching, sendo mais evidente a diferença do canteiro com pimenta comparado ao canteiro com lona amarela.


### Biomulching como controle de pragas

O biomulching contribui para reduzir a população de pulgões? E de moscas e lagartas?
...
```
Analisando o gráfico do número de outros insetos, pode-se visualizar uma tendencia para um menor numero no canteiro com lona amarela, nos demais canteiros não é possivil distinguir se há diferenças. Ou seja, não é possivél afirmar se o biomulching contribui para a redução do número de outros insetos. Sobre a presença de inimigos naturais, não foi registrado um grande número destes. O canteiro com biomulching foi o que teve maior presença de inimigos naturais, assim mostrando que não afeta negativamente estes organismos.
...
```{r grafico 3, eval=FALSE, include=FALSE}
df.armadilhas
...
Os números de plantas invasoras determinados no solo sem cobertura e com coberturas foram de uma maneira geral controlados significativamente pelo tipo de cobertura utilizado no cultivo da cultura. Percebe-se que o controle das plantas invasoras foi eficiente no solo com coberturas de plástico e bio-mulching, quando comparados com o solo sem cobertura dos canteiros de controle e pimenta.

Comparando os números de plantas invasoras determinados no solo sem cobertura e com coberturas de plástico e bio-mulching durante o período de teste, nota-se que o plástico reduziu a infestação das plantas invasoras em 85% e o bio-mulching em 67%. O fato de o bio-mulching ter tido um desempenho inferior, deve-se a sua degradação, que chegou a 82% no período de 84 dias, conforme mostra o gráfico anterior. O que através do tempo reduziu a área de proteção do solo, assim permitindo o crescimento de plantas invasoras.

[//] #esses valores em porcentagem foram obtidos por regra de três com o valor de controle como referência.

#### O biomulching apresenta um controle da umidade do solo semelhante ao mulching plástico?
